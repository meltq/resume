\documentclass{OpenCV}

% Header
\newlength{\hvspace}
\setlength{\hvspace}{-10pt}


\begin{document}

\begin{center}
    \noindent\textbf{\LARGE Tejas Vipin} \\
\end{center}
\vspace{\hvspace}
\hrulefill
\vspace{\hvspace}
\begin{center}
    \noindent SRM Institute of Science and Technology \textbullet\ Kattankulathur \textbullet\ Chennai,\, India \\
\end{center}
\vspace{\hvspace}
\vspace{-4pt}
\hrulefill
\vspace{-12pt}

% Links
\section*{Contact}
\faEnvelopeO\enskip \href{mailto:tejasvipin76@gmail.com}{tejasvipin76@gmail.com} \\
\faLinkedin\enskip\href{https://www.linkedin.com/in/tejas-vipin-b8788b24b/}{https://www.linkedin.com/in/tejas-vipin-b8788b24b/} \\
\faGithub\enskip\href{https://github.com/meltq}{https://github.com/meltq} \\

% Skills
\section*{Skills}
C{\textbackslash}C++, Python, Tensorflow, Pytorch, Linux, Device Drivers, Compilers, Embedded Systems, Machine Learning, Operating Systems, LLMs, AWS, Transformers, Git
% Current Position
% \section*{Current Position}
% \resumeSubHeadingListStart
% 
% \resumeSubheading
% {Tokyo Prefectural Jujutsu High School}{Tokyo,\, Japan}
% {Faculty}
% {Jul.\@ 2010 \textbf{-} Present} 

% \resumeSubHeadingListEnd

% Experience
\input{experience.tex}

% Open source
\input{opensource.tex}

% Research
\input{research.tex}

% Misc
\input{misc.tex}

% Education 
\input{education.tex}

\end{document}